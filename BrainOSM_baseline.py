    ...
    for epoch in range(1, e+1):
        ...
        for idx, data in enumerate(dataset):
            ...
            if epoch < 0:
                ...
                student.zero_grad()
                '''
                在mean-teacher架构中，这里生成的带噪声的数据adj_noise并不应该是和原始数据adj拼接在一起喂入student
                student中喂入的应该是根据概率决定的既有adj数据，也有adj_noise数据
                teacher中喂入的应该是adj数据，因为相比adj_noise，通过原始的adj生成的伪标签会更加可靠
                '''
